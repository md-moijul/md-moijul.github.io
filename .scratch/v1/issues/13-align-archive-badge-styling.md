...
The homepage badges use the default `Badge` variant which has a `bg-white/20` background and transparent border. The archive badges currently use `variant="outline"` with `bg-black/30` and `border-border/50`.
...
- [x] `ArchiveView.vue` badges use `bg-white/20` (achieved by removing `bg-black/30`).
